...
from sklearn.metrics import roc_auc_score, confusion_matrix, ConfusionMatrixDisplay  # Updated import
...
def go(args):
    ...
    test_data_path = run.use_artifact(args.test_data).file(root="./artifacts")
    ...
    cm = confusion_matrix(y_test, pipe.predict(X_test[used_columns]), normalize="true")  # Compute confusion matrix
    disp = ConfusionMatrixDisplay(confusion_matrix=cm, display_labels=pipe.classes_)  # Create display object

    fig_cm, sub_cm = plt.subplots(figsize=(10, 10))
    disp.plot(ax=sub_cm, cmap="viridis", values_format=".1f", xticks_rotation=90)  # Plot confusion matrix
    fig_cm.tight_layout()
